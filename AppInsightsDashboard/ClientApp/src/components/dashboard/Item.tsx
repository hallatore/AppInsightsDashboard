...
const ItemLink = styled(Link)<{ status: ItemStatus }>`
  ...
  animation: ${props =>
    props.status === ItemStatus.Error ? "BlinkAnimation 2s infinite" : null};
...
export default class Item extends React.Component<Props, State> {
  ...
  componentDidMount() {
    ...
    const intervalId = setInterval(
      this.ensureDataFetched.bind(this),
      120000 + 30000 * Math.random()
    );
  ...
  render() {
    ...
    if (status == ItemStatus.Disabled) {
      return (
        <ItemLinkDisabled
          to={`/${dashboardId}/Item/${groupIndex}/${itemIndex}`}
        >
          ...
          <Value>
            {value}
            <ValuePostfix>{item.postfix}</ValuePostfix>
          </Value>
        </ItemLinkDisabled>
      );
    }
